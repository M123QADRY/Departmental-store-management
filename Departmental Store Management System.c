...
#include <stdlib.h>
#include <string.h>
#define USERNAME "admin" //#define allows constant values to be declared for use throughout the code//
...
while (!feof(file)) //The feof() function indicates whether the end-of-file flag is set for the given stream .//
{
...
// Function to check the quantity during the sale product.
...
void updateProduct()//updates the product record//
{
...
{
printf("\n Updating data for the previous product %s\n", code);
...
// options();
// }
rewind(file); /*The rewind function sets the file position indicator for the stream pointed to by
